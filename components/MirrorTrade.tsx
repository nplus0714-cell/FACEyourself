  ...
  if (!user) {
    return <section className="mx-auto max-w-4xl pb-28 pt-4 fade-in md:pt-10">
      <div className="border border-[#D1D1C7] bg-[#F7F4EF] px-7 py-12 text-center md:px-16 md:py-20">
        ...
        <p className="mx-auto mt-6 max-w-2xl text-lg leading-[2] text-[#5F574F]">認識自己用 FACE，檢驗持倉用 RATE</p>
        <div className="mx-auto mt-10 max-w-xl border-y border-[#D1D1C7] py-7 text-left text-base leading-[2] text-[#5F574F]">
          <p>FACE 向內看見你的交易傾向；RATE 向外檢驗真實持倉。兩者落差太大時，往往就是焦慮的來源。</p>
          <p className="mt-3">登入後，你可以使用持股健檢、比例試算與 CSV 匯入工具。</p>
          <p className="mt-3">未來正式登入與 CRM 資料表接上後，診股紀錄會只屬於你自己的會員帳戶。</p>
        </div>
        <button type="button" onClick={onLogin} className="mt-10 bg-[#2D2D2D] px-9 py-4 text-base font-bold text-white transition hover:bg-black">登入後開始診股</button>
      </div>
    </section>;
  }
