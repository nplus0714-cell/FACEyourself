...
export const LandingInfo: React.FC<LandingInfoProps> = ({ onStartTest, onExploreTypes, onOpenContent, onAbout, isLoggedIn, hasSurvivalKitAccess, onRequireLogin, onOpenMemberHome }) => (
  <section className="mx-auto max-w-6xl space-y-14 pb-24 pt-14 md:space-y-24 md:pt-24" aria-label="FACE 介紹">
    <div className="border-y border-[#D1D1C7] py-12 text-center md:py-16">
      ...
      <p className="mx-auto mt-6 max-w-2xl text-base leading-[2] text-[#70665D] md:text-lg">FACE 交易心理測驗從機會、判斷、節奏與風險承受度四個方向，陪你看見自己的交易人格與交易習慣。</p>
      <a href="/about" onClick={(event) => { event.preventDefault(); onAbout(); }} className="mt-8 inline-block border-b border-[#2D2D2D] pb-1 text-sm font-bold text-[#2D2D2D]">認識 FACE →</a>
    </div>

    <div className="grid gap-px overflow-hidden border border-[#D1D1C7] bg-[#D1D1C7] md:grid-cols-3">
      <a href="/test" onClick={(event) => { event.preventDefault(); onStartTest(); }} className="group min-h-64 bg-[#F7F4EF] p-8 text-left transition hover:bg-white md:p-10">
        ...
        <p className="mt-5 text-base leading-[1.9] text-[#70665D]">FACE 交易心理測驗，用約 5 分鐘整理你在真實交易情境裡的第一反應。</p>
        <p className="mt-8 text-sm font-bold text-[#2D2D2D] group-hover:text-[#8C635B]">開始測驗 →</p>
      </a>
      <a href="/types" onClick={(event) => { event.preventDefault(); onExploreTypes(); }} className="group min-h-64 bg-white p-8 text-left transition hover:bg-[#F7F4EF] md:p-10">
      ...
      <a href="/watch" onClick={(event) => { event.preventDefault(); onOpenContent(); }} className="group min-h-64 bg-[#F7F4EF] p-8 text-left transition hover:bg-white md:p-10">
